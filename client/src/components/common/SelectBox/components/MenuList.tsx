    ...
    if (!isUserTypeArray(ListItem)) {
        if (searchValue.length !== 0) {
            const selectArray = ListItem.filter((item) => {
                ...
                return changeArray.splice(0, searchValue.length).join('') === searchValue
            })

            return [<S.List key={Math.floor(Math.random() * 10000)}>
                <UserList select={checkSelectItem(searchValue)} Item={{
                    user_id: Math.floor(Math.random() * 10000),
                    user_name: searchValue,
                    user_profile_img: searchValue,
                }}></UserList>
            </S.List>, ...selectArray.map((item) => <S.List key={item.user_id}><UserList
                select={checkSelectItem(item.user_name)} Item={item}/></S.List>)]
        }
        return ListItem.map((item) => <S.List key={item.user_id}><UserList
            select={checkSelectItem(item.user_name)} Item={item}/></S.List>)
    } else {
        if (searchValue.length !== 0) {
            const selectArray = ListItem.filter((item) => {
            ...
            return [
                <S.List key={Math.floor(Math.random() * 10000)}>
                    <TagList select={checkSelectItem(searchValue)} Item={{
                        tag_id: Math.floor(Math.random() * 10000),
                        tag_title: searchValue
                    }}></TagList>
                </S.List>, ...selectArray.map((item) => <S.List key={item.tag_id}><TagList
                    select={checkSelectItem(item.tag_title)} Item={item}/></S.List>)]
        }
        return ListItem.map((item) => <S.List key={item.tag_id}><TagList
            select={checkSelectItem(item.tag_title)}
            Item={item}/></S.List>)
    }
}
